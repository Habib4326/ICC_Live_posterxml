...
def get_poster(filename):
    try:
        ...
        search_year = year if year != "Unknown" else ""
        search_url = f"https://api.themoviedb.org/3/search/movie?api_key={TMDB_API_KEY}&query={quote(name)}&year={search_year}"
        ...
        try:
            ...
            for m in tree.getroot().findall("movie"):
                movies.append({
                    ...
                    "poster": m.find("poster").text,
                    "year": m.find("year").text if m.find("year") is not None else extract_year(m.find("title").text)
                })
        except: pass
    return movies
...
if __name__ == "__main__":
    while True:
        print("\n--- CODEX MULTI-SERVER FINDER (YEAR SORTED) ---")
